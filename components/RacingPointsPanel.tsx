...
/** Points system tables — used on the dedicated screen and the home Points tab. */
export function RacingPointsPanel({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <View style={styles.root}>
      {!compact ? <Text style={styles.title}>Points System</Text> : null}
      ...
      <View style={styles.tabsRow}>
        <TouchableOpacity
          ...
          activeOpacity={0.8}
        >
          <Text style={[styles.tabText, mainTab === 'standard' && styles.tabTextActive]}>
            Standard
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
      ...
      {mainTab === 'standard' ? (
        ...
        <>
          <Text style={styles.bonusDesc}>
            When your selection wins or places, you earn extra points on top of the standard points.
            The amount depends on the starting price (SP) range at the time of the result—higher
            ranges earn more bonus points, so a less-fancied pick that wins or places is rewarded
            more.
          </Text>
          ...
          <View style={styles.formatRow}>
            ...
            >
              <Text
                style={[styles.formatPillText, !showAsFraction && styles.formatPillTextActive]}
              >
                Decimal
              </Text>
            </TouchableOpacity>
          ...
          <View style={styles.bonusTabsRow}>
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.8}
            >
              <Text style={[styles.tabText, bonusTab === 'place' && styles.tabTextActive]}>
                Place
              </Text>
            </TouchableOpacity>
          </View>
          <View style={styles.table}>
            ...
            {(bonusTab === 'win' ? WIN_BONUS : PLACE_BONUS).map((r, i) => (
              <TableRow
                key={`${r.min}-${r.max}-${i}`}
                left={formatRange(r, showAsFraction)}
                right={`${r.points}`}
                styles={styles}
              />
            ))}
