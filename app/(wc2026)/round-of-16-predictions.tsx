...
export default function RoundOf16PredictionsScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      let cancelled = false;
      void (async () => {
        const h = await hasDownstreamKnockoutPredictions('r16');
        if (!cancelled) {
    ...
    const init = async () => {
      ...
      try {
        ...
        if (bracketData) {
          ...
          const { getR16Predictions, getAntePostLockedStatus } = await import('@/features/wc2026/services/async-predictions');
          const savedR16Predictions = await getR16Predictions();
          ...
          if (Object.keys(savedR16Predictions).length > 0) {
            // Load from AsyncStorage
            Object.entries(savedR16Predictions).forEach(([matchNumStr, pred]) => {
        ...
        Alert.alert('Error', 'Failed to load Round of 16 bracket');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleScoreChange = (matchNumber: number, team: 'home' | 'away', value: string, homeTeamId: string, awayTeamId: string) => {
    ...
    requestKnockoutEditWithDownstreamClear('r16', {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { saveR16Predictions } = await import('@/features/wc2026/services/async-predictions');
      
      // Convert predictions to AsyncStorage format
      const r16Predictions: Record<number, { match_number: number; home_score: number; away_score: number; predicted_winner_id: string | null }> = {};
      ...
      bracket.forEach((match) => {
        ...
        if (pred && pred.homeScore.trim() && pred.awayScore.trim()) {
          ...
          r16Predictions[match.matchNumber] = {
      ...
      Object.entries(r16Predictions).forEach(([matchNumStr, pred]) => {
      ...
      Alert.alert('Saved', 'Round of 16 predictions saved!');
    } catch (error) {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      // Navigate to Quarter Finals results screen
      router.push(
        wcHrefWithParams('/(wc2026)/quarter-finals-results', {
          bracket: JSON.stringify(bracket),
          predictions: JSON.stringify(r16Predictions),
        })
      );
    } catch (error) {
      console.error('Error saving predictions:', error);
  ...
  if (loading) {
    return (
      <View style={styles.container}>
        <WcKnockoutResultsHeader subtitle="Round of 16" onBack={handleBackPress} />
        <View style={styles.loadingContainer}>
          <ActivityIndicator size="large" color={theme.colors.accent} />
          <Text style={styles.loadingText}>Loading Round of 16 fixtures...</Text>
        </View>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <WcKnockoutResultsHeader
        subtitle="Round of 16 · Enter scores"
        progressText={progressLine}
        onBack={handleBackPress}
      />
      
      {/* Teams Advancing to Quarter Finals Header */}
      <View style={styles.winnersHeader}>
        <Text style={styles.winnersHeaderTitle}>Teams Advancing to Quarter Finals</Text>
        ...
        >
          ...
          <View style={styles.headerSection}>
            <Text style={styles.description}>
              Predict the outcome of each Round of 16 match. These matchups are determined by your Round of 32 predictions.
            </Text>
          </View>
