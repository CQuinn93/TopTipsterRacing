...
export default function QuarterFinalsPredictionsScreen() {
    ...
    const init = async () => {
      ...
      try {
        ...
        if (bracketData) {
          ...
          const { getQFPredictions, getAntePostLockedStatus } = await import('@/features/wc2026/services/async-predictions');
          const savedQFPredictions = await getQFPredictions();
          ...
          if (Object.keys(savedQFPredictions).length > 0) {
            // Load from AsyncStorage
            Object.entries(savedQFPredictions).forEach(([matchNumStr, pred]) => {
        ...
        Alert.alert('Error', 'Failed to load Quarter Finals bracket');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { saveQFPredictions } = await import('@/features/wc2026/services/async-predictions');
      
      // Convert predictions to AsyncStorage format
      const qfPredictions: Record<number, { match_number: number; home_score: number; away_score: number; predicted_winner_id: string | null }> = {};
      ...
      bracket.forEach((match) => {
        ...
        if (pred && pred.homeScore.trim() && pred.awayScore.trim()) {
          ...
          qfPredictions[match.matchNumber] = {
      ...
      Object.entries(qfPredictions).forEach(([matchNumStr, pred]) => {
      ...
      Alert.alert('Saved', 'Quarter Finals predictions saved!');
    } catch (error) {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      // Navigate to Semi Finals results screen
      router.push(
        wcHrefWithParams('/(wc2026)/semi-finals-results', {
          bracket: JSON.stringify(bracket),
          predictions: JSON.stringify(qfPredictions),
        })
      );
    } catch (error) {
      console.error('Error saving predictions:', error);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.winnersHeader}>
        <Text style={styles.winnersHeaderTitle}>Teams Advancing to Semi Finals</Text>
        ...
        >
          ...
          <View style={styles.headerSection}>
            <Text style={styles.description}>
              Predict the outcome of each Quarter Finals match. These matchups are determined by your Round of 16 predictions.
            </Text>
          </View>
