  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.table}>
        <TableRow left="Winner" right="5 pts" isHeader />
        <TableRow left="Place (2nd/3rd/4th)" right="1 pt" />
      </View>

      <Text style={[styles.sectionLabel, styles.sectionLabelSpaced]}>Win bonus (by SP range)</Text>
      <View style={styles.table}>
        <TableRow left="SP range" right="Bonus pts" isHeader />
        {WIN_BONUS.map((r) => (
          <TableRow key={r.range} left={r.range} right={`+${r.points}`} />
        ))}
      </View>
