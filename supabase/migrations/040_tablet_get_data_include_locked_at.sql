-- Include locked_at in quick-access selections payload so UI can enforce locked state.
...
begin
  ...
  select coalesce((select role from public.profiles where id = v_user_id), 'User')
    into v_role;
