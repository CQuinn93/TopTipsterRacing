...
function mapLeaderboardPredictionRaw(r: Record<string, unknown>): WcLeaderboardPredictionRow {
  return {
    id: String(r.id ?? ''),
...
/** Prediction row returned by competition / per-user prediction RPCs. */
  ...
  return arr
    .map((r) => {
      const row = r as {
        ...
        ante_points?: number | string;
        live_points?: number | string;
      };
      return {
        user_id: row.user_id,
        total_points: parseNum(row.total_points),
        ante_points: parseNum(row.ante_points),
        live_points: parseNum(row.live_points),
      };
    })
...
export async function wcFootballCompetitionPredictions(competitionId: string): Promise<WcLeaderboardPredictionRow[]> {
  ...
  if (error) throw new Error(error.message);
  const arr = parseRpcArray(data);
  return arr.map((raw) => mapLeaderboardPredictionRaw(raw as Record<string, unknown>));
}

/** One participant’s predictions for a mini-league (for lazy leaderboard drawers). */
export async function wcFootballUserCompetitionPredictions(
  competitionId: string,
  targetUserId: string
): Promise<WcLeaderboardPredictionRow[]> {
  const { data, error } = await rpc('wc_football_user_competition_predictions', {
    p_competition_id: competitionId,
